...
#+begin_src emacs-lisp
  ...
  (defalias 'personise
    (kmacro "A , I SPC <escape> j A , Y o u SPC ( s ) SPC <escape> j A , H e / s h e / i t SPC <escape> j A , W e SPC <escape> j A , Y o u SPC ( p l ) SPC <escape> j A , T h e y SPC <escape> 5 k"))
#+end_src
* Text functions
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (sp/leader-keys
    ...
    "r" '(sp/open-current-dir-yazi-float :which-key "yazi")
...
#+begin_src emacs-lisp
  (defun sp/open-current-dir-yazi-float ()
    (interactive)
    (let* ((lua
            (format
             "hl.dsp.exec_cmd(\"foot -e env EDITOR=\\\"emacsclient -n\\\" yazi %s\", { floating = true, size = {\"(monitor_w*0.8)\", \"(monitor_h*0.8)\"} })"
             default-directory)))
      (make-process
       :name "yazi"
       :command (list "hyprctl" "dispatch" lua)
       :noquery t)))
#+end_src
